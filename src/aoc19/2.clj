...
(defn find-input-for-output
  [program output]
  (for [i (range 99) j (range 99) :when (= output (first (get (run-program (restore-program program i j) 0 [0] nil) :program)))] (+ (* 100 i) j)))

(def part1 (first (get (run-program (restore-program core-program 12 2) 0 [0] nil) :program)))
